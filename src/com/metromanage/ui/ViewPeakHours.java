...
/**
 * ViewPeakHours - Admin dashboard for analyzing peak hours with modern UI.
 * Features: Analytics cards (Peak Hour, Peak Passengers, Current Hour), hourly breakdown table with filtering.
 */
public class ViewPeakHours extends JFrame {
    ...
    private JLabel lblPeakHourValue;
    private JLabel lblPeakHourSubtext;
    
    private JLabel lblPeakPassengersValue;
    private JLabel lblPeakPassengersSubtext;
    
    private JLabel lblCurrentHourValue;
    private JLabel lblCurrentHourSubtext;
    ...
    // Table components
    private JTable tblHourlyData;
    private DefaultTableModel tableModel;
    private JScrollPane scrollHourlyData;
    private JLabel lblShowingHours;
    ...
    private JPanel navPeakHours;
    private JPanel navLogout;

    // Data storage
    private ArrayList<HourlyData> hourlyDataList;
    ...
    private void initializeUI() {
        setTitle("View Peak Hours - MetroManage");
    ...
    private void createSidebar() {
        ...
        navBoardingTotals = createNavItem("Boarding Totals", false);
        pnlSidebar.add(navBoardingTotals);
        pnlSidebar.add(Box.createVerticalStrut(8));

        navPeakHours = createNavItem("Peak Hours", true);  // Active
        pnlSidebar.add(navPeakHours);
        pnlSidebar.add(Box.createVerticalStrut(8));
    ...
    private void createContentArea() {
        ...
        // Page title
        JLabel lblTitle = new JLabel("Peak Hours Analysis");
        ...
        // Bottom section: Hourly breakdown table
        createHourlyTable();
    ...
    private JPanel createAnalyticsCard(String title, String value, String subtext, Color accentColor) {
        ...
        if (title.equals("Peak Hour")) {
            lblPeakHourValue = lblValue;
        } else if (title.equals("Peak Passengers")) {
            lblPeakPassengersValue = lblValue;
        } else if (title.equals("Current Hour")) {
            lblCurrentHourValue = lblValue;
        }
        ...
        // Store references to update later
        if (title.equals("Peak Hour")) {
            lblPeakHourSubtext = lblSubtext;
        } else if (title.equals("Peak Passengers")) {
            lblPeakPassengersSubtext = lblSubtext;
        } else if (title.equals("Current Hour")) {
            lblCurrentHourSubtext = lblSubtext;
        }
    ...
    private void createHourlyTable() {
        ...
        JLabel tableTitle = new JLabel("Hourly Passenger Breakdown");
        tableTitle.setFont(getCustomFont(Font.BOLD, 20));
        tableTitle.setForeground(TEXT_PRIMARY);
        headerPanel.add(tableTitle, BorderLayout.WEST);

        // Right side: showing hours count
        lblShowingHours = new JLabel("Showing 24 hours");
        lblShowingHours.setFont(getCustomFont(Font.PLAIN, 13));
        lblShowingHours.setForeground(TEXT_MUTED);
        headerPanel.add(lblShowingHours, BorderLayout.EAST);

        pnlTableCard.add(headerPanel, BorderLayout.NORTH);

        // Create table
        String[] columns = {"Hour", "Passenger Count", "Percentage", "Status"};
        ...
        tblHourlyData = new JTable(tableModel);
        tblHourlyData.setFont(getCustomFont(Font.PLAIN, 13));
        tblHourlyData.setRowHeight(45);
        tblHourlyData.setShowGrid(false);
        tblHourlyData.setIntercellSpacing(new Dimension(0, 0));
        tblHourlyData.setSelectionBackground(new Color(240, 240, 240));
        tblHourlyData.setSelectionForeground(TEXT_PRIMARY);

        // Style table header
        JTableHeader header = tblHourlyData.getTableHeader();
        ...
        for (int i = 0; i < tblHourlyData.getColumnCount(); i++) {
            tblHourlyData.getColumnModel().getColumn(i).setCellRenderer(centerRenderer);
        }

        // Custom renderer for Passenger Count column (with bold text)
        tblHourlyData.getColumnModel().getColumn(1).setCellRenderer(new DefaultTableCellRenderer() {
            @Override
        ...
        tblHourlyData.getColumnModel().getColumn(3).setCellRenderer(new DefaultTableCellRenderer() {
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value,
                    boolean isSelected, boolean hasFocus, int row, int column) {
                Component c = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
                ...
                if (status.equals("Peak")) {
                    label.setForeground(new Color(211, 47, 47));  // Red for peak
                } else if (status.equals("High")) {
                    label.setForeground(new Color(255, 152, 0));  // Orange for high
                } else if (status.equals("Medium")) {
                    label.setForeground(new Color(76, 175, 80));  // Green for medium
                } else {
        ...
        // Set column widths
        tblHourlyData.getColumnModel().getColumn(0).setPreferredWidth(150);
        tblHourlyData.getColumnModel().getColumn(1).setPreferredWidth(200);
        tblHourlyData.getColumnModel().getColumn(2).setPreferredWidth(150);
        tblHourlyData.getColumnModel().getColumn(3).setPreferredWidth(150);

        scrollHourlyData = new JScrollPane(tblHourlyData);
        scrollHourlyData.setBorder(BorderFactory.createLineBorder(new Color(230, 230, 230)));
        scrollHourlyData.getViewport().setBackground(Color.WHITE);
        
        // Apply custom modern scrollbar
        scrollHourlyData.getVerticalScrollBar().setUI(new ModernScrollBarUI());
        scrollHourlyData.getHorizontalScrollBar().setUI(new ModernScrollBarUI());
        scrollHourlyData.getVerticalScrollBar().setUnitIncrement(16);

        pnlTableCard.add(scrollHourlyData, BorderLayout.CENTER);
    }
    ...
    private void applyFilters() {
        ...
        hourlyDataList = ridePersistanceHandler.getHourlyData(routeID, daysBack);
    ...
    private void updateTable() {
        ...
        for (HourlyData data : hourlyDataList) {
            totalPassengers += data.passengerCount;
        }
        
        // Add hourly data
        for (HourlyData data : hourlyDataList) {
            String hourStr = String.format("%02d:00 - %02d:59", data.hour, data.hour);
            double percentage = totalPassengers > 0 ? (data.passengerCount * 100.0 / totalPassengers) : 0;
            String percentageStr = String.format("%.1f%%", percentage);
            
            // Determine status based on passenger count
            String status;
            if (percentage >= 10) {
                status = "Peak";
        ...
        // Update showing hours label
        lblShowingHours.setText("Showing " + hourlyDataList.size() + " hours");
    }
    ...
    private void updateSummaryCards() {
        ...
        for (HourlyData data : hourlyDataList) {
            if (data.passengerCount > maxPassengers) {
                maxPassengers = data.passengerCount;
                peakHour = data.hour;
            }
        }
        
        if (peakHour != -1) {
            lblPeakHourValue.setText(String.format("%02d:00", peakHour));
            if (peakHour >= 6 && peakHour < 10) {
                lblPeakHourSubtext.setText("Morning rush");
            } else if (peakHour >= 17 && peakHour < 20) {
                lblPeakHourSubtext.setText("Evening rush");
            } else if (peakHour >= 12 && peakHour < 14) {
                lblPeakHourSubtext.setText("Lunch hour");
            } else {
                lblPeakHourSubtext.setText("Peak time");
            }
        } else {
            lblPeakHourValue.setText("N/A");
            lblPeakHourSubtext.setText("No data");
        }
        
        // Peak passengers
        lblPeakPassengersValue.setText(String.valueOf(maxPassengers));
        lblPeakPassengersSubtext.setText("At peak hour");
        
        // Current hour (assume current hour is the last data point or calculate based on actual time)
        java.time.LocalTime now = java.time.LocalTime.now();
        int currentHour = now.getHour();
        int currentHourPassengers = 0;
        
        for (HourlyData data : hourlyDataList) {
            if (data.hour == currentHour) {
                currentHourPassengers = data.passengerCount;
                break;
            }
        }
        
        lblCurrentHourValue.setText(String.valueOf(currentHourPassengers));
        lblCurrentHourSubtext.setText(String.format("Active at %02d:00", currentHour));
    }
